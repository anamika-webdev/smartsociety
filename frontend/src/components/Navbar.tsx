...
import { Bell, Calendar, CheckCircle2, Menu } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ title, onMenuClick }) => {
  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-white/80 dark:bg-darkCard/80 backdrop-blur-md border-b border-slate-200 dark:border-darkBorder flex items-center justify-between px-4 md:px-6 gap-3">
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <h1 className="text-base md:text-xl font-bold text-slate-800 dark:text-slate-100 font-sans tracking-tight truncate">
          {title}
        </h1>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-4 shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs font-semibold bg-slate-100 dark:bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-200/50 dark:border-slate-800">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-[calc(100vw-2rem)] max-w-xs sm:w-80 rounded-xl bg-white dark:bg-darkCard border border-slate-200 dark:border-darkBorder shadow-xl shadow-slate-200/50 dark:shadow-black/40 overflow-hidden z-50">
              <div className="px-4 py-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-darkBorder flex items-center justify-between">
                ...
                {unreadCount > 0 && (
                  <button onClick={markAllRead} className="text-[10px] text-sky-500 font-bold hover:underline">
                    Mark all read
                  </button>
                )}
              </div>
