...
export const Sidebar: React.FC<SidebarProps> = ({ collapsed, setCollapsed }) => {
  ...
  return (
    <aside 
      className={`fixed top-0 left-0 z-40 h-screen transition-all duration-300 border-r border-slate-200 dark:border-darkBorder bg-white dark:bg-darkCard flex flex-col justify-between
        ${collapsed ? 'w-20' : 'w-64'}`}
    >
      <div>
        {/* Brand Header */}
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-200 dark:border-darkBorder">
          {!collapsed ? (
            <div className="flex items-center gap-2 select-none">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-sky-500 to-indigo-600 flex items-center justify-center text-white font-bold shadow-md shadow-sky-500/35">
                <Shield size={18} />
              </div>
              <div>
            ...
            <div className="mx-auto w-8 h-8 rounded-lg bg-gradient-to-tr from-sky-500 to-indigo-600 flex items-center justify-center text-white font-bold shadow-md shadow-sky-500/35">
              <Shield size={18} />
            </div>
          )}
      ...
      <div className="p-3 border-t border-slate-200 dark:border-darkBorder space-y-3">
          ...
          user && (
            <div className="flex justify-center">
              <img 
                src={user.avatar} 
                alt={user.name}
                className="w-10 h-10 rounded-full border border-slate-200 dark:border-slate-800 object-cover cursor-pointer"
              />
            </div>
          )
        )}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            {theme === 'dark' ? (
              <>
                <Sun size={20} className="shrink-0 text-amber-500 animate-spin-slow" />
